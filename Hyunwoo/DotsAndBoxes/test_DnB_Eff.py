...
def test_apply_action_horizontal_no_box_then_turn_switch():
    ...
    assert_equal(eng.score, [0, 0], "점수 변화 없음")
...
def test_two_boxes_completed_on_shared_edge():
    ...
    assert_equal(eng.score[cur], 2, "두 박스 한 번에: 점수 +2")
...
def test_set_state_inconsistent_score_raises():
    eng = DotsAndBoxesEngine()
    apply_actions(eng, complete_box_actions(0, 0))
    s = eng.get_state()
    # 실제 완성 박스=1이지만 점수를 조작해서 불일치 만들기

    ## 이건 의도적으로 불일치를 만드는 거라 의미 X
    s["score"] = [0, 0]
    bad = DotsAndBoxesEngine()
    assert_raises(ValueError, bad.set_state, s)
...
TESTS = [
    ...
    test_set_state_inconsistent_score_raises,
