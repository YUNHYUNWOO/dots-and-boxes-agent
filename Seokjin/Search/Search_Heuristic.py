...
from Util import adjacent_boxes, box_edge_count, makes_third_edge 
...
def compelete_box(edges, action) -> bool:
    c, r, d = action
    hb, vb = edges
    for (bc, br) in adjacent_boxes(c, r, d):
        if box_edge_count(hb, vb, bc, br) == 3:
            # 지금 두면 박스가 완성됨
            return True
...
def make_double_cross(edges, action):
    ...
    b_list = adjacent_boxes(c, r, d)
    if len(b_list) == 2:
        return (box_edge_count(h, v, b_list[0][0], b_list[0][1]) == 2) and (box_edge_count(h, v, b_list[1][0], b_list[1][1]) == 2)
    else:
        return False
## Move_Ordering
