...
def complete_extension(info, out, action: Action):
    if out['is_box_completed']:
        return True

def give_away_extension(info, out, action: Action):
...
def move_ordering_v2(actions: list[Action], eng: DotsAndBoxesEngine, tt: TranspositionTable, depth:int, root_player:int):
